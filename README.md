...
phenotypes|string|Optional| phenotype should be formatted as follows and separated by vertical bars '\|' <br/>**PCR_VIVAX;arthropod infection status;Plasmodium vivax;present**<br/><br/>PCR_VIVAX is the protocol type (defined in study_protocols in config file) and, the next three values are the Observable, Attribute and Value that describe the phenotype (GMOD Chado style) and each value must be a term described in the study_terms section of the config file. Currently only pathogen infection status phenotypes are supported. It may be possible to support common IR phenotypes one day.
